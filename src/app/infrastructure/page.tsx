...
const services = [
  [
    ...
    "Inference",
    ":8000",
    "SGLang serves the local GPT-OSS-120B model through an OpenAI-compatible API.",
  ],
  [
    "02 / data plane",
    "Embedding",
    ":8001",
    "A local gte-Qwen2-1.5B service creates vectors and waits for inference readiness before launch.",
  ],
  [
    "03 / control plane",
    "Runtime",
    ":8003",
    "FastAPI publishes GPU, memory, temperature, power, container and request metrics to authorised admins.",
  ],
  [
    "04 / boundary",
    "Observer",
    "Unix socket",
    "The only Compose service with Docker-socket access exposes a small allowlisted read surface and no network interface.",
  ],
  [
    "05 / optional",
    "Speech",
    ":8006",
    "A profile-gated CPU service uses sherpa-onnx to synthesize authenticated WAV responses without text or audio logs.",
  ],
  [
    "06 / retention",
    "Log rotation",
    "no port",
    "A utility container bounds and compresses technical logs; application content is excluded by policy.",
  ],
  [
    "07 / outside Compose",
    "Supervisor",
    ":8002",
    "A separate privileged service prepares, verifies, switches, probes and compensates stack releases.",
  ],
  [
    "08 / host",
    "Coordinator",
    "systemd",
    "A narrow host process performs the final safe Supervisor handoff and crash recovery.",
  ],
] as const;
...
const isolationCards = [
  [
    "01 / no network",
    "Observer speaks Unix",
    "The Observer has no IP interface. Runtime reaches its allowlisted status and log methods through a shared read-only Unix-socket volume.",
  ],
  [
    "02 / reduced surface",
    "No exec or mutation",
    "The adapter does not offer a general Docker proxy, command execution, start, stop or arbitrary container selection.",
  ],
  [
    "03 / container hardening",
    "Drop what is not needed",
    "Read-only filesystem, all Linux capabilities dropped, no-new-privileges, PID/CPU/memory bounds and a small temporary filesystem.",
  ],
  [
    "04 / signed admin path",
    "Runtime verifies intent",
    "Short-lived Ed25519 JWTs bind issuer, audience, role, subject and time before an admin WebSocket can receive status or logs.",
  ],
  [
    "05 / separate privilege",
    "Supervisor is explicit",
    "The service that can update Docker remains outside Compose, behind the stronger super-admin audience and scope contract.",
  ],
  [
    "06 / verification",
    "Local harnesses, narrow CI",
    "Backend security contracts and recovery behavior use reproducible local test and Docker crash-recovery harnesses. A separate frontend GitHub Actions workflow performs secret scanning; it does not deploy the Spark.",
  ],
] as const;
...
const releaseSteps = [
  [
    "01",
    "Prepare",
    "Resolve read-only source commits and build a frozen local candidate without touching the running generation.",
  ],
  [
    "02",
    "Bind",
    "Confirmation carries a plan ID and hash tied to source, diff and current/previous release generations.",
  ],
  [
    "03",
    "Lock",
    "An operations lock and write-ahead log record intent before any state-changing step.",
  ],
  [
    "04",
    "Reconcile",
    "Services change in dependency order from immutable local release material and pinned dependency evidence.",
  ],
  [
    "05",
    "Qualify",
    "Readiness, health and smoke probes decide whether the new generation is accepted.",
  ],
  [
    "06",
    "Compensate",
    "A failed transaction returns to the exact job-start generation; Current and Previous remain locally addressable.",
  ],
] as const;
...
const benchmarkFacts = [
  [
    "Run",
    "03 Jun 2026",
    "Run ID 2026-06-03_225234_concurrency_instant; recorded from 22:52 to 23:51 CEST.",
  ],
  [
    "Mode",
    "Instant",
    "Synthetic German explanation prompts called the OpenAI-compatible inference API directly with low reasoning effort.",
  ],
  [
    "Load",
    "1 → 90",
    "Nine simultaneous-user levels, three repeats per level, with 30-second warm-up and cool-down periods.",
  ],
  [
    "Prompt",
    "500 tokens",
    "Fixture de_explanation_medium_001, temperature 0.2; answer quality and browser performance were outside the measurement.",
  ],
  [
    "Result",
    "723 / 723",
    "All requests in this particular run completed successfully. That is evidence for the run—not a future availability promise.",
  ],
  [
    "Build",
    "f92491e",
    "Frontend commit f92491e on PHP 8.4.20; the manifest records the target model alias as school-ui.",
  ],
] as const;
      ...
      <main>
        ...
        <section className={atlas.blueSection}>
          ...
          <div className={atlas.metricGrid}>
            <article className={atlas.metric}>
              ...
              <p>
                tokens per request, preventing one request from consuming the
                whole KV-cache pool
              </p>
            </article>
            <article className={atlas.metric}>
              ...
              <strong>15</strong>
              <p>
                maximum running inference requests in the current school-tuned
                configuration
              </p>
            </article>
            <article className={atlas.metric}>
              ...
              <strong>8,192</strong>
              <p>
                tokens with up to 64 embedding requests in the configured local
                service
              </p>
            </article>
            <article className={atlas.metric}>
              ...
              <strong>0.80</strong>
              <p>
                of the inference service memory budget reserved by the current
                SGLang launch profile
              </p>
            </article>
          </div>
          <div className={atlas.formulaBand}>
            ...
            <div>
              <code>
                available ≈ unified memory − weights − KV cache − runtime
                headroom
              </code>
              <p>
          ...
          <div className={atlas.benchmarkGrid}>
            ...
            <figure className={atlas.benchmarkPlate}>
              ...
              <figcaption>
                ...
                optimizing one without the other would misdescribe classroom
                load.
              </figcaption>
            </figure>
          </div>
          ...
          <div className={atlas.split}>
            <div className={atlas.stickyCopy}>
              ...
              <ul className={atlas.annotationList}>
                <li>
                  <span>01</span> GPU, RAM, power and temperature snapshots
                </li>
                <li>
                  <span>02</span> Readiness for inference, embedding and
                  optional speech
                </li>
                <li>
                  <span>03</span> Request, latency and token counters held in
                  memory
                </li>
                <li>
                  <span>04</span> Allowlisted container log tails through the
                  Observer
                </li>
              </ul>
            </div>
        ...
        <section className={atlas.paperSection}>
          ...
          <div className={atlas.formulaBand}>
            ...
            <div>
              <code>
                commit ⇔ source_bound ∧ material_verified ∧ probes_pass
              </code>
              <p>
